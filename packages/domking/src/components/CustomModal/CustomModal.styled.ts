...
export const StyledOverlay = styled.div<{ colorOption: Color, backgroundOption: Background, blurOption: Blur }>`
  ...
  background-color: ${(props) => {
    ...
    const transparency = props.backgroundOption.map(option => option[1]).includes("transparent");
  ...
  backdrop-filter: ${(props) => {
    ...
    if (props.backgroundOption.map(option => option[1]).includes("blured") && blurValRadio !== "manual") {
        switch (blurValRadio) {
            case "light":
                return css`blur(2px)`
            case "medium":
    ...
    } else if (props.backgroundOption.map(option => option[1]).includes("blured") && blurValRadio === "manual") {
